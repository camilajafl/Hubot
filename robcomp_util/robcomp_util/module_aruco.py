...
class Aruco3d():
    def __init__(self, camera_matrix=None, camera_distortion=None):
        ...
        if camera_matrix is None:
            self.camera_matrix = "robcomp_util/robcomp_util/config/cameraMatrix_realsense.txt"
            self.camera_distortion = "robcomp_util/robcomp_util/config/cameraDistortion_realsense.txt"
        else:
            self.camera_matrix = self.camera_matrix
            self.camera_distortion = self.camera_distortion
...
def main():
    Arucos = Aruco3d()
    bgr = cv2.imread("robcomp_util/robcomp_util/aruco.jpeg")
